...
const CATS=['Alojamiento','Desayuno','Almuerzo','Cena','Refrigerio','Estacionamiento','Peaje','Servicios','Viajes','Otros'];
...
export default function ConfirmStep({extracted,isManual,identity,onSubmit,onBack}){
  ...
  async function handleSubmit(e){
    ...
    try{
      const payload={...form,titular:identity.nombre,tarjeta:identity.tarjeta,sociedad:identity.sociedad,modo_carga:isManual?'Manual':'Foto',_fileBase64:extracted?._fileBase64,_fileMime:extracted?._fileMime,_fileName:extracted?._fileName};
    ...
    <div style={styles.card}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={ss}>
          ...
          <Field label="CUIT (opcional)"><input style={styles.input} value={form.cuit} onChange={e=>set('cuit',e.target.value)} placeholder="XX-XXXXXXXX-X"/></Field>
          ...
          {isFacA&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
            <Field label="Subtotal"><input style={styles.input} type="number" step="0.01" value={form.subtotal} onChange={e=>set('subtotal',e.target.value)} placeholder="0.00"/></Field>
